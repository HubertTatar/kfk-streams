...
object OffsetsForTime extends App {
  ...
  topicPartOffset.foreach(tpo => {
    ...
    val date = LocalDateTime.ofInstant(Instant.ofEpochMilli(offsetAndTimestamp.timestamp()), TimeZone
      .getDefault().toZoneId());

    val zoned = ZonedDateTime.ofInstant(Instant.ofEpochMilli(offsetAndTimestamp.timestamp()), ZoneId.of("UTC"))

    println(s"Offset: ${offsetAndTimestamp.offset()}, timestamp: ${offsetAndTimestamp.timestamp()}: date: ${date}, zoned: ${zoned}")
  })
